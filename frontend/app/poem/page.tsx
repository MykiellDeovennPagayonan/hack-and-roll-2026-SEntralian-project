...
export default function PoemCamera() {
  ...
  const capturePhoto = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:8000/poem/image", {
        method: "POST",
      ...
      if (data.success && data.poem) {
        sessionStorage.setItem("generatedPoem", data.poem);
        stopCamera();
        router.push("/poem/result");
      } else {
        ...
        {cameraState === "capturing" && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/50">
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="text-xl text-white">Generating poem...</div>
            </div>
          </div>
        )}
      </div>
      ...
      {(cameraState === "ready" || cameraState === "capturing") && (
        <div className="absolute bottom-8 left-0 right-0 flex justify-center">
          <button
            ...
            className="flex h-20 w-20 items-center justify-center rounded-full border-4 border-white bg-white/20 transition-all hover:bg-white/30 active:scale-95 disabled:opacity-50"
          >
