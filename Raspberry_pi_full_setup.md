...
# Start xdg-desktop-portal (needed for camera access)
echo "$(date): Starting xdg-desktop-portal..." >> "$LOG"
/usr/libexec/xdg-desktop-portal &
sleep 2  # Give it time to start

# Create kiosk profile directory (incognito won't save permissions, so we need this)
mkdir -p ~/.config/chromium-kiosk

# Remove Chromium crash warnings from kiosk profile
sed -i 's/"exited_cleanly":false/"exited_cleanly":true/' ~/.config/chromium-kiosk/Default/Preferences 2>/dev/null
sed -i 's/"exit_type":"Crashed"/"exit_type":"Normal"/' ~/.config/chromium-kiosk/Default/Preferences 2>/dev/null
...
chromium-browser \
  ...
  --use-gl=swiftshader \
  --user-data-dir=/home/sibato/.config/chromium-kiosk \
...
# Test camera capture from command line
v4l2-ctl --device=/dev/video0 --stream-mmap --stream-count=1 --stream-to=test.jpg
file test.jpg  # Should show "JPEG image data"
```

**Important Chromium flags for camera in kiosk mode:**
- `--use-fake-ui-for-media-stream` - Auto-grants camera permission without prompt (required for kiosk)

The app automatically handles USB webcams by falling back from mobile-style constraints (`facingMode`) to basic video capture if needed.
...
The first request after boot is slow because Ollama loads the model into RAM.

**Warm up the model on boot:**

Add to your kiosk script before launching Chromium:

```bash
# Warm up Ollama model (add to start-kiosk.sh)
echo "$(date): Warming up Ollama model..." >> "$LOG"
curl -s http://localhost:11434/api/generate -d '{"model":"gemma3:4b","prompt":"hi","stream":false}' > /dev/null
echo "$(date): Model warm-up complete" >> "$LOG"
```

**Check system resources:**

```bash
# Check memory usage (gemma3:4b needs ~4GB RAM)
free -h
...
1. Use Pi 5 with 8GB RAM (4GB is tight for gemma3:4b)
2. Increase swap size if needed:
   ...
   ```
3. Keep model loaded by setting in ollama service:
   ```bash
   # In /etc/systemd/system/ollama.service, add:
   Environment=OLLAMA_KEEP_ALIVE=24h
   ```
4. Consider a smaller model like `gemma3:1b` for faster responses
